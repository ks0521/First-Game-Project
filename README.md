...
> **Kamikakushi (가칭)** — Unity 기반 1인칭 호러 어드벤처 팀 프로젝트

## 1. 프로젝트 소개

`First-Game-Project`는 Unity로 개발된 3D 호러 어드벤처 게임 프로젝트입니다. 플레이어는 마을/숲/집 등의 공간을 탐색하며 아이템을 수집하고, 상호작용 오브젝트를 활용해 목표(Objective)를 단계적으로 달성합니다. 진행 중에는 체력(HP)과 정신력(Sanity) 관리가 필요하며, 선택과 진행 상태에 따라 엔딩 분기가 발생합니다.
...
1. **탐색**: 맵을 이동하며 단서와 상호작용 가능한 오브젝트를 찾습니다.
2. **상호작용**: 오브젝트에 접근해 조사/사용/획득 등의 행동을 수행합니다.
3. **인벤토리 관리**: 획득한 아이템을 상황에 맞게 사용해 진행을 해금합니다.
4. **생존 관리**: HP/Sanity 상태를 관리하며 위협(몬스터/환경)을 회피합니다.
5. **목표 달성 및 분기**: Objective를 갱신하며 진행하고, 조건에 따라 엔딩이 달라집니다.

## 4. 주요 기능

- **진행 단계(Progress Step) 기반 게임 플로우**
  - 튜토리얼 → 빌리지 → 포레스트 → 엔딩 구간으로 이어지는 스텝 기반 진행
- **상호작용 시스템**
...
- **몬스터 시스템**
  - 몬스터 베이스 클래스 + 타입별 동작(예: Zombie, Ghoul, Banshee)
- **저장/불러오기 시스템**
...
- **기획**: `이름` — 세계관/레벨 설계, 진행 단계 정의
- **클라이언트(게임플레이)**: `이름` — 플레이어 조작, 상호작용, 진행 로직
- **클라이언트(UI/UX)**: `이름` — HUD, 인벤토리, 설정/가독성 개선
- **클라이언트(AI/콘텐츠)**: `이름` — 몬스터 로직, 이벤트, 씬 연출
- **아트/레벨디자인**: `이름` — 환경 구성, 프리팹 배치, 비주얼 톤 조정
...
### 참고
- 기존 프로젝트 표기는 `First-Game-Project`이며, 문서 상 게임명은 `Kamikakushi`로 작성했습니다.
- 팀명/프로젝트명/링크/이미지는 실제 정보로 교체 후 배포하는 것을 권장합니다.
